...
(defn- create-test-data [f]
  (binding [context/*test-users* +fake-users+
    ...
    (let [owner (test-helpers/getx-test-users :owner)
          handler (test-helpers/getx-test-users :approver1)
          handler2 (test-helpers/getx-test-users :approver2)
          reviewer (test-helpers/getx-test-users :reviewer)]
      (test-helpers/assert-no-existing-data!)
      (rems.service.test-data/create-test-api-key!)
    ;; Users
      #_(test-helpers/create-user! (get test-users/+fake-user-data+ "owner"))
      #_(test-helpers/create-user! (get test-users/+fake-user-data+ "carl"))
      #_(test-helpers/create-user! (get test-users/+fake-user-data+ "handler"))
      #_(test-helpers/create-user! (get test-users/+fake-user-data+ "reporter") :reporter)
      #_(test-helpers/create-user! {:userid "applicant" :organizations [{:organization/id "default"}]})
      #_(test-helpers/create-user! (get test-users/+fake-user-data+ "alice"))
      #_(test-helpers/create-user! (get test-users/+fake-user-data+ "developer"))
      (rems.service.test-data/create-test-users-and-roles!)
    ;; Organizations
      ...
      (test-helpers/create-organization! {:actor owner
                                          ...
                                          :organization/owners [{:userid (test-helpers/getx-test-users :organization-owner2)}]
                                          :organization/review-emails []})
    ;; Forms, workflows etc.
    ;; These should match the rems.service.test-data closely enough
    ;; so that one can use also development mode and dev db
    ;; with the tests
      (test-helpers/create-workflow! {:type :workflow/master})
      (let [link (test-helpers/create-license! {:actor owner
            ...
            form (test-helpers/create-form! {:actor owner
                                             ...
                                             :form/fields rems.service.test-data/all-field-types-example})
          ...
          ;;                                                                                                            :values ["csc"]}]}]}})
            item-id1 (test-helpers/create-catalogue-item! {:form-id form
                                                           :workflow-id wfid
                                                           :title {:en "Default workflow" :fi "Oletustyövuo"
          ...
          ;;                                                                                                :sv "Standard blankettarbetsflöde med DUO-koder"} :resource-id duo-resource})
            applicant (test-helpers/create-user! {:userid "applicant"
